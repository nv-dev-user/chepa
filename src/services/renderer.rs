...
pub fn render_player_position(player: &LivingEntity, zones: &Vec<Zone>) {
    ...
    let north_name = north
        ...
        .map(|zone| format!("North: {}", zone.get_entity().get_name()))
        .unwrap_or_default();
    let east = player.get_zone().get_east_zone_id();
    let east_name = east
        .and_then(|i| searchZoneById(i, zones))
        .map(|zone| format!("East: {}", zone.get_entity().get_name()))
        .unwrap_or_default();
    let west = player.get_zone().get_west_zone_id();
    let west_name = west
        .and_then(|i| searchZoneById(i, zones))
        .map(|zone| format!("West: {}", zone.get_entity().get_name()))
        .unwrap_or_default();
    let south = player.get_zone().get_south_zone_id();
    let south_name = south
        .and_then(|i| searchZoneById(i, zones))
        .map(|zone| format!("South: {}", zone.get_entity().get_name()))
        .unwrap_or_default();
    ...
    let center = render_player(&player, [north, east, south, west]);
...
fn render_player(player: &LivingEntity, zones: [Option<u32>; 4]) -> String {
